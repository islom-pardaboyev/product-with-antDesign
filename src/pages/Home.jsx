      ...
      res.data.map((item, index) => {
        ...
        item.action = (
          <div className="flex space-x-2">
            ...
            <button onClick={() => navigate(`/update/${item.id}`)} className="w-[30px] bg-green-500 text-white border-green-500 hover:border-green-500 hover:bg-transparent  hover:text-green-500 duration-300 h-[30px] rounded-full border">
              <EditOutlined />
            </button>
          </div>
        );
    ...
    <div className="p-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[15px] text-slate-500">Products ({products.length})</p>
        </div>
